...
<body>
    ...
    // Adjust the queries as needed based on your database schema
    $result = $conn->query("SELECT * FROM district");
    displayData($result, "District");

    $result = $conn->query("SELECT * FROM schools");
    displayNestedData($result, "Schools", $conn);

    $conn->close();

    // Function to display data in a table
    function displayData($result, $tableName) {
    ...
    function displayNestedData($result, $tableName, $conn) {
        echo "<h3>$tableName Data</h3>";
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo "<div>";
                ...
                $studentsResult = $conn->query("SELECT * FROM students WHERE school_id = $schoolId");
                displayData($studentsResult, "Students");
...
<!-- below part from   from ruf 2 page  -->


<h2>Display Data below part from   from ruf 2 page</h2>
...
$result = $conn->query("SELECT * FROM district");
displayDataa($result, $conn, "District");

$conn->close();

function displayDataa($result, $conn, $tableName) {
    echo "<h3>$tableName Data</h3>";
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
